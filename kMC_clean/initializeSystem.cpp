...
void initialGrid(std::vector<State> *stateVector,std::map<int,int> *outCoordmap,std::array<int,2> *outVcoord )
{
    ...
    for (int pos = 0; pos < grid_array.size(); ++pos)
    {
        ...
        if (position.xcoordinate == side_length/2 && position.ycoordinate == side_length/2)
        {
            ...
            std::cout<<"Vacancy Located: {"<<(*outVcoord)[0]<<","<< (*outVcoord)[1]<< "}"<<std::endl;
        }
...
void initialGridNew(std::vector<State> *stateVector,std::map<int,int> *outCoordmap,std::array<int,2> *outVcoord,int vacancy_xcoord, int vacancy_ycoord)
{
    ...
    for (int pos = 0; pos < grid_array.size(); ++pos)
    {
        ...
        position.ycoordinate = grid_array[pos][1];
        //assign state
        if (position.xcoordinate == vacancy_xcoord && position.ycoordinate == vacancy_ycoord)
        {
            //place vacancy in middle
            //std::cout<< "VACANCY\n";
            position.state_type = 'V';
...
void visualizeGrid (std::vector<State> *stateVector)
{
    ...
    std::cout<<(*stateVector).capacity()<<std::endl;
    ...
    std::vector<State> row4;
    row4.reserve(g_side_length);
    std::vector<State> row5;
    row5.reserve(g_side_length);
    std::vector<State> row6;
    row6.reserve(g_side_length);
    std::vector<State> row7;
    row7.reserve(g_side_length);
    std::vector<State> row8;
    row8.reserve(g_side_length);
    for (int i = 0; i < stateVector ->size(); ++i)
    {
        switch ((*stateVector)[i].ycoordinate)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                row4.push_back((*stateVector)[i]);
                break;
            case 5:
                row5.push_back((*stateVector)[i]);
                break;
            case 6:
                row6.push_back((*stateVector)[i]);
                break;
            case 7:
                row7.push_back((*stateVector)[i]);
                break;
            case 8:
                row8.push_back((*stateVector)[i]);
                break;
            default:
    ...
    //vector of variables and map back and forth
}
